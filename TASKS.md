...
- [ ] **Partial returns on sales with cost components** — a sale can now draw COGS from several items (a remote included with a VHS — see [`docs/features/sales.md`](docs/features/sales.md#cost-components-multi-item-cogs)). `record_return` reverses `inventory_movements` **LIFO by unit count** with no notion of which item a unit belongs to, so returning 1 of 2 units on such a sale would restore the *component's* stock instead of the sold item's. Mitigated client-side for now: `ProcessReturnModal` detects components (any movement whose lot's `item_id` ≠ the sale's) and pins those sales to a **full** return, which reverses every movement and is correct. Real fix: teach `record_return` to reverse only movements matching `sales.item_id` on a partial return, taking components only on a full one — requires redeploying `record_return` (already undeployed-drifted, see the task above, so bundle the two deploys).
