...
- [x] **Breakdown Inventory (box opening) flow** (2026-08-03) — the other half of [that spec](docs/superpowers/specs/2026-06-23-box-opening-and-grading-design.md): breaks an existing inventory lot (sealed box, bundle) into many single-card lots with basis allocated by relative FMV / equal / specific $ (`box_openings` table, `openBox`/`deleteBoxOpening`, `OpenBoxModal`, `BoxOpeningDetailSlideOver` — UI says "Breakdown Inventory"; table/function names unchanged). Revised same-day to pick the box from inventory (depleting its lot) instead of typing a name/cost and posting a new transaction — the box's cost is already deducted at purchase, so a second deduction at open time would double-count it. See [`docs/features/inventory.md`](docs/features/inventory.md#opening-a-box). **`box_openings` table applied to Supabase; the follow-up `source_lot_id`/`quantity` migration is not yet confirmed applied** — run `supabase db push` (`supabase/migrations/20260803130000_box_openings_source_lot.sql`) before using it; see the Deployment note in [`docs/supabase-schema.md`](docs/supabase-schema.md).
