...
- [x] **Open Box flow** (2026-08-03) — the other half of [that spec](docs/superpowers/specs/2026-06-23-box-opening-and-grading-design.md): one sealed-box purchase splits into many single-card lots with basis allocated by relative FMV / equal / specific $ (`box_openings` table, `openBox`/`deleteBoxOpening`, `OpenBoxModal`, `BoxOpeningDetailSlideOver`). See [`docs/features/inventory.md`](docs/features/inventory.md#opening-a-box). **Migration not yet applied to any Supabase project** — run `supabase db push` (`supabase/migrations/20260803120000_box_openings.sql`) before using it; see the Deployment note in [`docs/supabase-schema.md`](docs/supabase-schema.md).
