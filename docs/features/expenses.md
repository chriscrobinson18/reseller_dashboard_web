...
- Inline category change: clicking the `CategoryBadge` in a row opens `CategoryDropdown`, a fixed-position popover positioned via CSS vars (`--dd-top`/`--dd-left`) set from the clicked element's bounding rect — not a `<select>`. Mutates via local `updateCategory`, invalidates `['transactions']`.
...
- Category dropdown here is a second, separate implementation from the list-view `CategoryDropdown` (absolutely positioned relative to the button, not fixed/viewport-positioned) — if you change category-picker UX, both need updating.
