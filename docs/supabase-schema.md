...
### `inventory_movements`
Audit trail row created by `record_sale` per lot depleted by a sale. Read-only from the web client (`sale.inventory_movements`); join shape: `{ id, quantity, inventory_lots: { unit_cost, item_id, items: { name } } }`. `quantity * unit_cost` summed across a sale's movements = that sale's COGS.

**Movements may span several items.** The table constrains only `(sale_id, inventory_lot_id)` — nothing ties a movement's lot to `sales.item_id`. That's what makes *cost components* possible: a sale of a VHS can also consume a remote bought to sell with it, and both costs land in one COGS figure. A movement whose `inventory_lots.item_id` differs from the sale's `item_id` **is** a cost component — there's no separate flag, and none is needed. Written by `add_sale_cost_components` (below); see [`features/sales.md`](features/sales.md#cost-components-multi-item-cogs).
...
- **`add_sale_cost_components`** — **a Postgres RPC, not an edge function** ([migration](../supabase/migrations/20260726120000_sale_cost_components.sql)). `supabase.rpc('add_sale_cost_components', { p_sale_id, p_components: [{ item_id, quantity }] })`. FIFO-depletes each component item's lots (oldest `created_at` first, `FOR UPDATE`) and inserts `inventory_movements` rows against an existing sale, so extra items included in a sale contribute to its COGS. Returns `{ ok, movements_created, unfulfilled }` where `unfulfilled` maps `item_id → quantity short`; a shortfall flags the sale `oversold` rather than raising, matching `record_sale`. SECURITY DEFINER with an `auth.uid()` check, granted to `authenticated` only. **Not idempotent** — purely additive, so calling it twice double-depletes; `recordSale`/`recordBundleSale` call it exactly once, right after `record_sale`. Chosen over extending the `record_sale` edge function so the depletion is genuinely transactional and locked (same rationale as `reverse_sale`).
