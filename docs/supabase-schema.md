...
| `source` | `'manual'` or `'csv_import'` (added 2026-08-27 — see below) |
| `refund_transaction_id`, `return_shipping_transaction_id` | nullable FKs to `transactions`, added 2026-08-27 by the `csv_return_reconciliation` migration. **Null for manual returns** (which insert fresh transaction rows, as before). **Set for `source='csv_import'` returns** — they record which existing `csv_import` transaction rows `record_return` re-tagged (rather than inserting new ones), so `reverse_return` can un-tag exactly those rows instead of deleting real imported bank/marketplace history. See [`docs/superpowers/specs/2026-08-27-csv-return-reconciliation-design.md`](superpowers/specs/2026-08-27-csv-return-reconciliation-design.md). |

The return's side effects live outside this table: `record_return` restores `inventory_lots.quantity_remaining` (LIFO), updates the sale's `refunded_*`/`return_status`/`inventory_status`, and posts a `returns_allowances` refund `transactions` row (+ a `shipping_postage` row for the return label if given), all carrying `related_sale_id`. See the edge-function notes below and [data-flows.md](data-flows.md#revenue-net-of-returns).

**CSV reconciliation (2026-08-27):** when `refund_transaction_id`/`return_shipping_transaction_id` are given, `record_return` **updates** those existing `csv_import` rows in place (setting `related_sale_id`, and for the refund row, `schedule_c_category = 'returns_allowances'`) instead of inserting synthetic ones — the manual path's insert-and-delete-on-reverse behavior is unchanged. See [`docs/features/settings.md`](features/settings.md#return-reconciliation) for the review-queue UI and [csvReturns.ts](../src/lib/csvReturns.ts) for the detection/matching logic.
...
- **`record_sale`** ([source](../supabase/functions/record_sale/index.ts)) — `supabase.functions.invoke('record_sale', { body: { item_id, quantity, sale_price, platform, sold_at, source, external_order_id } })`. Inserts the `sales` row, FIFO-depletes `inventory_lots.quantity_remaining`, creates `inventory_movements` rows. Returns `{ sale_id, inventory_status, unfulfilled_quantity }`. **Does not persist `fees`/`shipping_cost`/`net_payout`** — the web client writes those onto the `sales` row in a follow-up `.update()` call (see `recordSale` in mutations.ts). Verified end-to-end in the P0 pass; the source has a contract header comment spelling this out. The 2026-08-02 `deleted_at is null` FIFO-query fix (below) is **deployed** as of 2026-08-25.
- **`record_return`** (v21 deployed; **v2 CSV-reconciliation changes below are local-only, not yet deployed** — see Deployment note below, [source](../supabase/functions/record_return/index.ts)) — called by `ProcessReturnModal` via `recordReturn` (manual path) and `ReconcileReturnModal` via the same `recordReturn` wrapper (CSV path). Fixed in the P0 pass: reverses `inventory_movements` LIFO and restores `quantity_remaining` on the original source lots at the lots' original `unit_cost` (no more fake new-lot-at-sale-price). Refund row: `schedule_c_category: 'returns_allowances'`, `related_sale_id`; manual path (`source: 'manual'`) inserts a new `transactions` row (`amount: -refund_amount`, `type: 'refund'`), CSV path (`refund_transaction_id` given) instead re-tags that existing `csv_import` row in place. Same insert-vs-re-tag split for the return-shipping row (`schedule_c_category: 'shipping_postage'`), keyed on `return_shipping_cost` vs `return_shipping_transaction_id`. Guards against double-applying a CSV row already linked to a sale (409).
- **`reverse_return`** (deployed 2026-08-25; **v2 CSV-reconciliation changes below are local-only, not yet deployed** — see Deployment note below, [source](../supabase/functions/reverse_return/index.ts)) — called by `ProcessReturnModal`'s edit flow via `reverseReturn`. Re-depletes inventory FIFO for the return's quantity (mirrors `record_sale`'s lot-selection algorithm, including the same 2026-08-02 `deleted_at is null` fix), decrements the sale's `refunded_quantity`/`refunded_amount` and recomputes `return_status`/`inventory_status`. Manual returns (`source: 'manual'`): deletes the refund + return-shipping `transactions` rows (`related_sale_id` + `type = 'refund'`). CSV-reconciled returns (`source: 'csv_import'`, added 2026-08-27): **un-tags** `returns.refund_transaction_id`/`return_shipping_transaction_id` instead (clears `related_sale_id`; restores the refund row's `schedule_c_category` to `'payout'`) rather than deleting real imported transaction rows. Either way, deletes the `returns` row. Assumes at most one active return per sale (current UI scope). Not atomic — sequential service-role calls, same as `record_return`.
